...
export default function WebsiteScanForm(): React.JSX.Element {
  ...
  const getPerformanceLabel = (score: number): PerformanceLabel => {
    ...
    if (score >= 50) return { label: 'Needs Improvement', color: 'text-accent' };
    ...
    <>
      ...
      {isLoading && (
        <div className="text-center mt-12">
          <div className="relative flex items-center justify-center">
            <div className="w-16 h-16 border-4 border-t-accent border-gray-300 rounded-full animate-spin" style={{
          width: '4rem', // w-16
          height: '4rem', // h-16
      ...
      {isError && (
        <p className="text-center text-red-400 mt-12 text-lg sm:text-xl">
          Error: {(error as ApiError)?.data?.message || (error as ApiError)?.message || 'Something went wrong'}
        </p>
      )}
        ...
        <div className="max-w-4xl mx-auto mt-12">
          ...
          <div className="mb-12">
            ...
            {data.results.errors.length > 0 ? (
              <ul className="space-y-6">
                {data.results.errors.map((error: ScanResult['results']['errors'][0], index: number) => (
                  <li
                    key={index}
                  ...
                  >
                    ...
                    {error.element && (
                      <div className="mt-4">
                        <p className="text-gray-300">
                          <strong className="text-white">Element Selector:</strong> {error.element.selector}
                        </p>
          ...
          <div>
            ...
            {data.results.alerts.length > 0 ? (
              <ul className="space-y-6">
                {data.results.alerts.map((alert: ScanResult['results']['alerts'][0], index: number) => (
                  <li
                    key={index}
