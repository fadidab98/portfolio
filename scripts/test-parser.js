...
dom.querySelectorAll('head link').forEach((link, i) => console.log(`Link ${i + 1}: ${link.toString()}`));
...
scripts.forEach((script, i) => {
  ...
  if (content.includes('rel:"stylesheet"')) {
    console.log(`Found stylesheet in script ${i + 1}: ${content.slice(0, 200)}...`);
    script.textContent = content.replace(
      /\["\$","link","[^"]+",\{"rel":"stylesheet","href":"\/_next\/static\/css\/[^"]+","precedence":"next".*?\}\]/g,
      ''
    );
    console.log(`Cleaned script ${i + 1}: ${script.textContent.slice(0, 200)}...`);
  }
});

const noscriptTags = dom.querySelectorAll('head noscript');
console.log(`Found ${noscriptTags.length} <noscript> tags`);
noscriptTags.forEach((tag, i) => console.log(`<noscript> ${i + 1}: ${tag.toString()}`));

const head = dom.querySelector('head');
const newNoscript = parse('<noscript><link rel="stylesheet" href="/_next/static/css/1eac7ab8b6dbc3a4.css"></noscript>');
head.appendChild(newNoscript);
console.log('After adding new <noscript>:');
dom.querySelectorAll('head noscript').forEach((tag, i) => console.log(`<noscript> ${i + 1}: ${tag.toString()}`));
